...
type ResearchResult = {
  learnings: string[];
  visitedUrls: string[];
  sources: string[]; // Добавляем sources для хранения URL
};
...
export async function deepResearch({
      ...
      limit(async () => {
        try {
          ...
          const sources = result.data
            .map(item => item.url) // Получаем URL
          ...
          return {
            learnings: [],
            visitedUrls: [],
            sources: [], // Если ошибка, источников нет
          };
        }
      }),
    ),
  ...
  return {
    ...
    sources: [...new Set(results.flatMap(r => r.sources))], // Собираем все уникальные источники
  };
}